...
<div>
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 mb-2">Search Results: "{{ $query }}"</h1>
        <p class="text-gray-600">{{ $posts->total() }} {{ Str::plural('result', $posts->total()) }}</p>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        @forelse($posts as $post)
            <a href="{{ route('blog.show', $post) }}?highlight={{ urlencode($query) }}" class="block">
        ...
        @empty
            <p class="text-gray-600 col-span-2">No posts found matching your search.</p>
        @endforelse
    </div>
